 ...
 * 7. TikTokOptimizer + WhatsAppOptimizer started/stopped alongside VPN listeners
 *    for QUIC stall detection, NAT keep-alive, and call stall warnings.
 */
...
import TikTokOptimizer  from '../services/TikTokOptimizer';
import WhatsAppOptimizer from '../services/WhatsAppOptimizer';
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    // FIX 7: Start app-level optimizers alongside VPN listeners
    TikTokOptimizer.start();
    WhatsAppOptimizer.start();

    // Optional: surface stall warnings in the event log
    const unTikTokStall = TikTokOptimizer.on('tiktokStall', (info) => {
      if (info) log('TikTok loading — please wait…');
    });
    const unTikTokRecovering = TikTokOptimizer.on('tiktokRecovering', () => {
      log('TikTok stalled — attempting reconnect…');
    });
    const unWaReady = WhatsAppOptimizer.on('whatsappReady', () => {
      log('WhatsApp calls ready ✓');
    });
    const unWaCallStall = WhatsAppOptimizer.on('whatsappCallStall', (info) => {
      if (info) log('WhatsApp call unstable — reconnecting…');
    });
    const unWaMediaStall = WhatsAppOptimizer.on('whatsappMediaStall', (info) => {
      if (info) log('WhatsApp download stalled — retry in WhatsApp.');
    });
    ...
    return () => {
      // FIX 7: Stop optimizers on cleanup
      TikTokOptimizer.stop();
      WhatsAppOptimizer.stop();
