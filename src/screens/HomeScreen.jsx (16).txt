...
// App list shown in the selector
const APPS = [
  { label: 'TikTok',    service: TikTokService   },
  { label: 'WhatsApp',  service: WhatsAppService  },
  { label: 'Facebook',  service: FacebookService  },
  { label: 'Instagram', service: InstagramService },
  { label: 'YouTube',   service: YouTubeService   },
  { label: 'Twitter',   service: TwitterService   },
  { label: 'Chrome',    service: ChromeService    },
];
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    attachListeners(activeService);
    return () => clearListeners();
  }, [selectedAppIdx]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDisconnect = useCallback(async () => {
    ...
    setIdle();
    log('Disconnected.');
  }, [log, clearTimers, setIdle, activeService]);
  ...
  const handleStartHost = async () => {
    try {
      setRole('host');
      setConnecting();
      log(`Starting host on ${networkType} via ${APPS[selectedAppIdx].label}...`);
      await activeService.startAsHost(networkType);
    } catch (e) {
  ...
  const handleStartClient = async () => {
    ...
    try {
      setRole('client');
      setConnecting();
      log(`Joining with code ${code} via ${APPS[selectedAppIdx].label}...`);
      await activeService.startAsClient(code);
    } catch (e) {
  ...
  return (
    <SafeAreaView style={s.safeArea}>
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        {/* ── App selector ── */}
        <Text style={s.sectionLabel}>SELECT APP</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.appRow}>
          {APPS.map((app, idx) => (
            <TouchableOpacity
              key={app.label}
              style={[s.appBtn, selectedAppIdx === idx && s.appBtnActive]}
              onPress={() => { if (!isActive) setSelectedAppIdx(idx); }}
              disabled={isActive}>
              <Text style={[s.appBtnText, selectedAppIdx === idx && s.appBtnTextActive]}>
                {app.label}
              </Text>
            </TouchableOpacity>
          ))}
...
const s = StyleSheet.create({
  ...
  sectionLabel: { color: '#4a6080', fontSize: 10, letterSpacing: 3, marginBottom: 8 },

  appRow:        { flexDirection: 'row', marginBottom: 14 },
  appBtn:        { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20,
                   borderWidth: 1, borderColor: BORDER, backgroundColor: CARD,
                   marginRight: 8 },
  appBtnActive:  { borderColor: CYAN, backgroundColor: '#0d2030' },
  appBtnText:    { color: '#4a6080', fontWeight: '600', fontSize: 13 },
