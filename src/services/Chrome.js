...
class ChromeVpnService {
  ...
  async startAsHost(netType = 'WiFi') {
    ...
    await VpnModule.startVpn(RELAY_URL, '', 'host', this.hostId, netType, '', JSON.stringify(CHROME_PACKAGES));
  }
  ...
  async startAsClient(accessCode) {
    ...
    await VpnModule.startVpn(RELAY_URL, accessCode.toUpperCase(), 'client', '', '', deviceId, JSON.stringify(CHROME_PACKAGES));
  }
  ...
  _scheduleReconnect() {
    ...
    this.reconnectTimer = setTimeout(async () => {
      try {
        if (this._stopping) return;
        const deviceId = await this.getDeviceId();
        if (this.role === 'host') await VpnModule.startVpn(RELAY_URL, '', 'host', this.hostId, this.netType, '', JSON.stringify(CHROME_PACKAGES));
        else if (this.role === 'client' && this.accessCode) await VpnModule.startVpn(RELAY_URL, this.accessCode, 'client', '', '', deviceId, JSON.stringify(CHROME_PACKAGES));
