/**
 * src/services/Facebook.js — NetShare
 *
 * Self-contained VPN service for Facebook.
 * Drop this file in and import FacebookService anywhere you need it.
 * No shared state with other app services — fully independent.
 *
 * Packages tunneled:
 *   com.facebook.katana   — Facebook main
 *   com.facebook.lite     — Facebook Lite
 *   com.facebook.android  — Facebook alternate package
 ...
 *     Facebook uses Play Services for push notifications and auth.
 *   com.google.android.webview / com.android.webview
 *     Facebook in-app browser and link previews use Android WebView.
 *
 * Special network handling:
 *   Port 443  — HTTPS/QUIC (news feed, video), 600s
 *   Port 80   — HTTP fallback, 600s
 *   Port 5228 — FCM push (Messenger notifications), 900s
 *   Port 3478 — STUN for Messenger voice/video calls, 600s
 *   Port 53   — DNS, 5s
 */
...
export const FACEBOOK_PACKAGES = [
  'com.facebook.katana',
  'com.facebook.lite',
...
class FacebookVpnService {
  ...
  async startAsHost(netType = 'WiFi') {
    ...
    await VpnModule.startVpn(RELAY_URL, '', 'host', this.hostId, netType, '', JSON.stringify(FACEBOOK_PACKAGES));
  }
  ...
  async startAsClient(accessCode) {
    ...
    await VpnModule.startVpn(RELAY_URL, accessCode.toUpperCase(), 'client', '', '', deviceId, JSON.stringify(FACEBOOK_PACKAGES));
  }
  ...
  _scheduleReconnect() {
    ...
    this.reconnectTimer = setTimeout(async () => {
      try {
        if (this._stopping) return;
        const deviceId = await this.getDeviceId();
        if (this.role === 'host') await VpnModule.startVpn(RELAY_URL, '', 'host', this.hostId, this.netType, '', JSON.stringify(FACEBOOK_PACKAGES));
        else if (this.role === 'client' && this.accessCode) await VpnModule.startVpn(RELAY_URL, this.accessCode, 'client', '', '', deviceId, JSON.stringify(FACEBOOK_PACKAGES));
  ...
  async stop() {
    this._stopping = true;
    clearTimeout(this.reconnectTimer); this.reconnectTimer = null; this.reconnectTries = 0;
...
export default new FacebookVpnService();
