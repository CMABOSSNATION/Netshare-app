/**
 * src/services/YouTube.js — NetShare
 *
 * Self-contained VPN service for YouTube + Spotify.
 * Drop this file in and import YouTubeService anywhere you need it.
 ...
 *     YouTube uses Play Services for DRM (Widevine), auth, and push.
 *   com.google.android.webview / com.android.webview
 *     YouTube's in-app browser for linked content.
 *
 * Special network handling:
 *   Port 443 UDP — QUIC/HTTP3 (YouTube adaptive streaming + Spotify CDN), 600s
 *                  16 MB socket buffer — YouTube 4K bursts up to 25 Mbps
 *   Port 443 TCP — HTTPS fallback, 600s
 *   Port 80  TCP — HTTP fallback, 600s
 *   Port 5228    — FCM push (YouTube notifications), 900s
 *   Port 53      — DNS, 5s
 */
...
export const YOUTUBE_PORTS = {
  443:  600_000,  // QUIC/HTTPS — YouTube ABR streaming + Spotify CDN
  80:   600_000,  // HTTP fallback
  5228: 900_000,  // FCM push — YouTube notifications
  53:   5_000,
  853:  5_000,
  123:  10_000,
};
...
class YouTubeVpnService {
  ...
  async startAsHost(netType = 'WiFi') {
    ...
    await VpnModule.startVpn(RELAY_URL, '', 'host', this.hostId, netType, '', JSON.stringify(YOUTUBE_PACKAGES));
  }
  ...
  async startAsClient(accessCode) {
    ...
    await VpnModule.startVpn(RELAY_URL, accessCode.toUpperCase(), 'client', '', '', deviceId, JSON.stringify(YOUTUBE_PACKAGES));
  }
  ...
  _scheduleReconnect() {
    ...
    this.reconnectTimer = setTimeout(async () => {
      try {
        if (this._stopping) return;
        const deviceId = await this.getDeviceId();
        if (this.role === 'host') await VpnModule.startVpn(RELAY_URL, '', 'host', this.hostId, this.netType, '', JSON.stringify(YOUTUBE_PACKAGES));
        else if (this.role === 'client' && this.accessCode) await VpnModule.startVpn(RELAY_URL, this.accessCode, 'client', '', '', deviceId, JSON.stringify(YOUTUBE_PACKAGES));
...
export default new YouTubeVpnService();
